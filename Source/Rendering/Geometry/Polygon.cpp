...
Polygon::Polygon(const std::string & filename)
{
	...
	for (int i = 0; i < numVertices; i++) {
		Vertex vertice;
		infile.read((char*)&vertice, sizeof(Vertex));
...
bool Polygon::intersects(Line& line, Point& interseccion)
{
	for (int i = 0; i < this->getNumVertices(); ++i) {
		if (this->getEdge(i).intersects(line, interseccion)) {
			return true;
		}
	}
	return false;
...
bool Polygon::intersects(RayLine& ray, Point& interseccion)
{
	for (int i = 0; i < this->getNumVertices(); ++i) {
		if (this->getEdge(i).intersects(ray, interseccion)) {
			return true;
		}
	}
	return false;
...
bool Polygon::intersects(SegmentLine& segment, Point& interseccion)
{
	for (int i = 0; i < this->getNumVertices(); ++i) {
		if (this->getEdge(i).intersects(segment, interseccion)) {
			return true;
		}
	}
	return false;
...
bool Polygon::pointInConvexPolygonGeo(Point& point)
{
	...
	for (int i = 0; i < this->getNumVertices(); i++) {
		Vertex v = this->getVertexAt((i));
		Vertex v2 = this->getVertexAt((i + 1) % this->getNumVertices());
		if (point.triangleArea2(v,v2) < 0) {
			return false;
		}
	}
	return true;
...
void Polygon::save(const std::string& filename)
{
	...
	outfile.write((char*)&numVertices, sizeof(int));
	outfile.write((char*)this->_vertices.data(), numVertices * sizeof(Vertex));
	outfile.close();
}
