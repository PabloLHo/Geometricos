...
Voxelization::Voxelization(TriangleModel *modelo, vec3 _tam, int tipo) {
	tam = _tam;
	AABB aabb_modelo = modelo->getAABB();
	xmax = aabb_modelo.getMax().getX(); ymax = aabb_modelo.getMax().getY(); zmax = aabb_modelo.getMax().getZ();
	xmin = aabb_modelo.getMin().getX(); ymin = aabb_modelo.getMin().getY(); zmin = aabb_modelo.getMin().getZ();
	numX = ((xmax - xmin) / tam[0]) + 1;
	numY = ((ymax - ymin) / tam[1]) + 1;
	numZ = ((zmax - zmin) / tam[2]) + 1;

	float auxX, auxY, auxZ;
	auxX = xmin; auxY = ymin; auxZ = zmin;
...
Voxelization::Voxelization(float _xmax, float _ymax, float _zmax, float _xmin, float _ymin, float _zmin, vec3 _tam) {
	tam = _tam; xmax = _xmax; ymax = _ymax; zmax = _zmax;
...
void Voxelization::lineaBarrido(std::vector<Triangle3d> triangulos) {
	...
	for (int i = 0; i < numY; i++) {
		float nivelMaxY = this->voxeles[0][i][0]->getMax().getY();
		float nivelMinY = this->voxeles[0][i][0]->getMin().getY();
...
std::vector<Voxel*> Voxelization::obtenerVoxeles(AABB aabb_ti) {
	...
	std::vector<std::pair<Voxel*,vec3>> solucion;
...
bool Voxelization::insideVoxel(Voxel *voxel, Vect3d v) {
	return v.getX() >= voxel->getMin().getX() && v.getX() <= voxel->getMax().getX() && 
		v.getY() >= voxel->getMin().getY() && v.getY() <= voxel->getMax().getY() &&
		v.getZ() >= voxel->getMin().getZ() && v.getZ() <= voxel->getMax().getZ();
}
		...
		for (int x = 0; x < numY; x++) {
			for (int y = 0; y < numZ; y++) {
				if (this->getVoxeles()[i][x][y]->getFormato() == formatin) {
					...
					vector.push_back(vec3(centro.getX(), centro.getY(), centro.getZ()));
				}
			}
		}
	}
	vec3* positions = new vec3[vector.size()];
	vec3* color = new vec3[vector.size()];
...
bool Voxelization::rayTraversal(Ray3d& r, std::vector<Voxel*>& v) {
	...
	if (!ray_intersects_grid) return 0;
	...
	while(glm::all(glm::greaterThanEqual(vec3(current_X_index, current_Y_index, current_Z_index),vec3(1,1,1))) && 
		glm::all(glm::lessThanEqual(vec3(current_X_index, current_Y_index, current_Z_index), vec3(numX,numY,numZ)))){
...
bool Voxelization::rayBoxIntersection(Ray3d& r, float& tMin, float& tMax, float t0, float t1) {
	float tYMin, tYMax, tZMin, tZMax;
	const float x_inv_dir = 1 / r.getDirection().getX();
	if (x_inv_dir >= 0) {
	...
	const float y_inv_dir = 1 / r.getDirection().getY();
	if (y_inv_dir >= 0) {
	...
	const float z_inv_dir = 1 / r.getDirection().getZ();
	if (z_inv_dir >= 0) {
...
Voxelization* Voxelization::AND(Voxelization& vox) {
	...
	vec3 tamNuevo(BasicGeometry::max2(this->getXMax() - this->getXMin(), vox.getXMax() - vox.getXMin()), BasicGeometry::max2(this->getYMax() - this->getYMin(), vox.getYMax() - vox.getYMin()), BasicGeometry::max2(this->getZMax() - this->getZMin(), vox.getZMax() - vox.getZMin()));
	...
	res = new Voxelization(BasicGeometry::max2(this->getXMax(), vox.getXMax()), BasicGeometry::max2(this->getYMax(), vox.getYMax()),
		...
		BasicGeometry::min2(this->getYMin(), vox.getYMin()), BasicGeometry::min2(this->getZMin(), vox.getZMin()), vec3(tam.x));

	num_voxeles = vec3(res->numX, res->numY, res->numZ);
	int xV, yV, zV;
	int x2, y2, z2;
