...
AABB TriangleModel::getAABB() {
    ...
    for (int i = 0; i < this->getVertices()->size(); i++) {
        ...
        Vect3d vertice = this->getVertices()->at(i);
...
void TriangleModel::rayTraversalExh(Ray3d& r, std::vector<Vect3d>& p, std::vector<Triangle3d*>& t) {
    std::vector<Triangle3d> faces = this->getFaces();

    for (int i = 0; i < faces.size(); i++) {

        Triangle3d face = faces.at(i);
        Vect3d aux;

        if (face.ray_tri(r, aux)) {
            p.push_back(aux);
            t.push_back(&face);
        }
...
bool TriangleModel::pointIntoTMexh(Vect3d& v) {
    ...
    Vect3d d1(RandomUtilities::getUniformRandom(minBoundaries.x, maxBoundaries.x), RandomUtilities::getUniformRandom(minBoundaries.x, maxBoundaries.x), RandomUtilities::getUniformRandom(minBoundaries.x, maxBoundaries.x));
    Ray3d r1(v, d1);
    Vect3d d2(RandomUtilities::getUniformRandom(minBoundaries.x, maxBoundaries.x), RandomUtilities::getUniformRandom(minBoundaries.x, maxBoundaries.x), RandomUtilities::getUniformRandom(minBoundaries.x, maxBoundaries.x));
    Ray3d r2(v, d2);

    rayTraversalExh(r1, p, t);
    rayTraversalExh(r2, p2, t2);

    if (p.size() % 2 == 0 && p2.size() % 2 == 0) {
        return false;
    }
    else if (p.size() % 2 == 1 && p2.size() % 2 == 1) {
        return true;
    }
    else {
        ...
        Vect3d d3(RandomUtilities::getUniformRandom(minBoundaries.x, maxBoundaries.x), RandomUtilities::getUniformRandom(minBoundaries.x, maxBoundaries.x), RandomUtilities::getUniformRandom(minBoundaries.x, maxBoundaries.x));
        Ray3d r3(v, d3);

        rayTraversalExh(r3, p3, t3);
...
bool TriangleModel::pointIntoTMVoxel(Vect3d& v) {

    Voxelization voxelizacion;

    vec3 pos((v.getX() - voxelizacion.getXMin()) / voxelizacion.getTam().x, (v.getY() - voxelizacion.getYMin()) / voxelizacion.getTam().y,
        (v.getZ() - voxelizacion.getZMin()) / voxelizacion.getTam().z);
    Voxel *voxel = voxelizacion.getVoxeles()[pos.x][pos.y][pos.z];
    if (voxel->getFormato() == Formato_Voxel::BLANCO) {
        delete voxel;
        return false;
    }
    else if (voxel->getFormato() == Formato_Voxel::NEGRO) {
        delete voxel;
        return true;
    }
    delete voxel;
    return pointIntoTMexh(v);
}